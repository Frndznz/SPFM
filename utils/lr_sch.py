...
class CosineRestartsWithDecay(CosineAnnealingWarmRestarts):
    """
    A Cosine Annealing Warm Restarts scheduler where the peak learning rate 
    decays by a factor of 'gamma' at each restart.
    (Note: This implementation is optimized for T_mult=1)
    """
    ...
    def get_lr(self):
        # Calculate how many restarts have occurred
        num_restarts = self.last_epoch // self.T_0
        
        # Apply the gamma decay to the peak (base_lr)
        return [
            self.eta_min + (base_lr * (self.gamma ** num_restarts) - self.eta_min) * (1 + math.cos(math.pi * self.T_cur / self.T_i)) / 2
